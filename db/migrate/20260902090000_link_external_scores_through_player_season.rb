class LinkExternalScoresThroughPlayerSeason < ActiveRecord::Migration[7.1]
  ...
  def change
    ...
    # No default is possible for a reference column, and none is needed: per the ticket,
    # dev/test data gets wiped and no backfill runs, so the table is empty at migration time.
