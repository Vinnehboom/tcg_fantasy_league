...
- Read-only: no code execution, no specs, no mutation testing — see references/reviewer.md. The commit gate (Developer) and CI (Gatekeeper Phase 5) already confirm the suite is green; the Reviewer judges correctness and spec quality by reading, not by running anything.
