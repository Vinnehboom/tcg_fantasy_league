# Drafting app — project context (cached 2026-08-06)

url: https://app.notion.com/p/3a94af79fc0180f68bd6e37f39723fc8

# Multi-game player DB + placement-based scoring
**Status:** planning (approved direction, not yet implemented) · **Date:** 2026-07-26
The Kanban board holds the 26 PR-sized stories (Epics A–D) with Status / Epic / Priority / Depends-On and per-card done criteria. This page is the durable context + session summary behind it.
...
- **Adapter is dependency-injected** into `ExternalData::Interface`, `OfflineAdapter` default (frozen-hash registry rejected). **← this is A-2.**
...
- **The fetch half is a per-host client plus an injected retry policy.** `ExternalData::JsonApiClient.new(base_uri:, retry_policy:)` is a plain reusable object — one instance per external host, many `get_json(path:, query:)` calls — with all timeout/backoff/`Retry-After` logic in an injected `ExternalData::RetryPolicy`. GET-only; retryable set is deliberately narrow (timeouts + 429, no 5xx). `ApplicationService`/`.call` was considered and rejected for it (A-1 Checkpoint 2). A-2/B-1/D-2 build on this contract.
...
- `config/credentials/test.key` gitignored/absent-in-fresh-session caveat (see full page if it recurs).
