...
- **`config/credentials/test.key`** is gitignored and absent in a fresh session — see full recovery steps on the source page if needed. Never commit a regenerated `test.yml.enc`.

## Open items

None blocking. Confirm at build time: queue adapter choice; exact result-page markup and rating JSON field names; the placement→points weighting curve (tunable). (Not directly relevant to A-1, which has no queue/results scope.)
